...
def SIDD_test(model, opt):
    ...
    # load info
    files = scipy.io.loadmat(os.path.join(opt['datasets']['test_1']['dataroot_Noisy'], 'BenchmarkNoisyBlocksSrgb.mat')) # 读取mat格式文件
    ...
    for i in range(nImages):
        ...
        for k in range(nBlocks):
            ...
            if opt['self_ensemble']:
                ensemble = opt['self_ensemble']
                print(f'self_ensemble:{ensemble}')
                ...
                print(curr_time)
                print(sum(timings[2:])/len(timings[2:])) if len(timings)>=3  else None
            else:
                model.test()
